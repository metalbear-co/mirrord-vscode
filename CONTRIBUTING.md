...
Publishing is handled by `.github/workflows/release.yaml`. It runs on manual dispatch and on merged PRs into `main` whose source branch matches `release/<version>` or `releases/<version>`. Before publishing, it verifies that the version currently in `package.json` does not already have a GitHub release. It then builds and publishes the extension, creates the GitHub release using the version currently in `package.json` as the release tag, updates the tracking tags, and finally dispatches the `metalbear-co/infra` workflow `update-vscode-version.yml` with the released version.
